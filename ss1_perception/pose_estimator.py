...
# Updated from /camera/camera/color/camera_info
FX = 908.1590576171875
FY = 907.3975830078125
CX = 647.8463745117188
CY = 366.2890319824219
...
COLOURS = {
    'red': {
        'ranges': [
            (np.array([0, 150, 110]), np.array([10, 255, 210])),
        ],
        'bgr': (0, 0, 220)
    },
    'yellow': {
        'ranges': [(np.array([10, 100, 80]), np.array([35, 255, 255]))],
        'bgr': (0, 215, 255)
    },
    'blue': {
        'ranges': [(np.array([98, 170, 40]), np.array([115, 255, 255]))],
        'bgr': (220, 80, 0)
    },
}
...
class PoseEstimatorNode(Node):
    ...
    def __init__(self):
        ...
        # Last known stable pose per colour per cluster: {colour: {cluster_id: (X,Y,Z,yaw,timestamp)}}
    ...
    def callback(self, colour_msg, depth_msg):
        ...
        for colour_name in COLOUR_ORDER:
            ...
            # Sort clusters by Y position for consistent left-to-right ordering
            valid_clusters = [
