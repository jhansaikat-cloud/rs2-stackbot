...
class PoseEstimatorNode(Node):
    ...
    def _find_or_create_cluster(self, colour, X, Y, Z):
        ...
        # Hard cap — max cubes per colour
        max_per_colour = {'red': 3, 'yellow': 2, 'blue': 1}
        ...
        # Only create new cluster if under the cap
        if len(clusters) < max_per_colour.get(colour, 3):
            new_id = len(clusters)
            clusters[new_id] = []
            return new_id

        # Over cap — assign to nearest cluster
        nearest_id = min(clusters.keys(),
            key=lambda cid: math.sqrt(
                (X - clusters[cid][-1][0])**2 +
                (Y - clusters[cid][-1][1])**2 +
                (Z - clusters[cid][-1][2])**2
            ) if clusters[cid] else float('inf')
        )
        return nearest_id
